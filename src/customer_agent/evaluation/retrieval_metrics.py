...
Convention: LLMTestCase.metadata["gold_article_ids"] holds the GOLD article
ids, and LLMTestCase.retrieval_context holds the RETRIEVED article ids in rank
order (article-level, best rank across tool calls, deduped). LLMTestCase.context
belongs to the LLM judge (gold article texts), not to these metrics.
"""
...
class _RankedRetrievalMetric(BaseMetric):
    ...
    def measure(self, test_case: LLMTestCase, *args, **kwargs) -> float:
        gold = set((test_case.metadata or {}).get("gold_article_ids", []))
